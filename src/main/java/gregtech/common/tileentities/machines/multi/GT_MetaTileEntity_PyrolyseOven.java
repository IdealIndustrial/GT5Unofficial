...
public class GT_MetaTileEntity_PyrolyseOven extends GT_MetaTileEntity_MultiBlockBase {
	...
	Block CASING = GregTech_API.sBlockCasings6;
	private final int CASING_INDEX = 192;
	private final int CASING_META = 0;
    ...
    public ITexture[] getTexture(IGregTechTileEntity aBaseMetaTileEntity, byte aSide, byte aFacing, byte aColorIndex, boolean aActive, boolean aRedstone) {
        if (aSide == aFacing) {
            return new ITexture[]{Textures.BlockIcons.casingTexturePages[1][64], new GT_RenderedTexture(aActive ? Textures.BlockIcons.OVERLAY_FRONT_PYROLYSE_OVEN_ACTIVE : Textures.BlockIcons.OVERLAY_FRONT_PYROLYSE_OVEN)};
        }
        return new ITexture[]{Textures.BlockIcons.casingTexturePages[1][64]};
    }
                    ...
                    if ((i != -2 && i != 2) && (j != -2 && j != 2)) {// innerer 3x3 ohne hoehe
                        if (h == 0) {// innen boden (kanthal coils)
                            ...
                            if ((!addInputToMachineList(tTileEntity, CASING_INDEX)) && (!addMufflerToMachineList(tTileEntity, CASING_INDEX))) {
                                ...
                                if (aBaseMetaTileEntity.getMetaIDOffset(xDir + i, h, zDir + j) != CASING_META) {
                                    return false;
                                }
                            }
                        } else {// innen air
