...
public abstract class GT_MetaTileEntity_DrillerBase extends GT_MetaTileEntity_MultiBlockBase {
        ...
        for (int xOff = -1 + back.offsetX; xOff <= 1 + back.offsetX; xOff++) {
            for (int zOff = -1 + back.offsetZ; zOff <= 1 + back.offsetZ; zOff++) {
                ...
                if (!addMaintenanceToMachineList(tTileEntity, casingTextureIndex)
                        ...
                        && !checkCasingBlock(xOff, 0, zOff)
            )
                    return false;
            }
        }
        if (!checkHatches()) {
            sendErrorLocalString("multiblock.error.driller.invalidhatches");
            return false;
        }
        if (GT_Utility.getTier(getMaxInputVoltage()) < getMinTier()) {
            sendErrorLocalString("multiblock.error.driller.invalidvoltage");
            return false;
        }
        //check tower
        for (int yOff = 1; yOff < 4; yOff++) {
            if (!checkFrameBlock(back.offsetX + 1, yOff, back.offsetZ)
                    ...
                    || !checkCasingBlock(back.offsetX, yOff, back.offsetZ)
            )
                return false;
        }
        return true;
    ...
    protected boolean checkCasingBlock(int xOff, int yOff, int zOff) {
        return checkBlockAndMetaOffset(xOff, yOff, zOff, casingBlock, casingMeta, true);
    }
    ...
    protected boolean checkFrameBlock(int xOff, int yOff, int zOff) {
        return checkBlockAndMetaOffset(xOff, yOff, zOff, GregTech_API.sBlockMachines, frameMeta, false);
    }

    protected boolean checkBlockAndMetaOffset(int xOff, int yOff, int zOff, Block block, int meta, boolean hatch) {
        return !checkNotBlock(block, meta, xDrill + xOff, yDrill + yOff, zDrill + zOff, hatch);
    }
