...
public class GT_Loader_Item_Block_And_Fluid
    ...
    public void run() {
        ...
        GregTech_API.sBlockGlass = new GT_Block_Glass_Casings("gt.blockglass");
    ...
    //TODO: REMOVE EVERYTHING AFTER TESTS
    private void addSpamFluids() {
        if (!GT_DebugConfig.addTrashFluids)
            return;
        Random r = new Random();
        if (r.nextBoolean())
            return;
        for (int i = 0, l = r.nextInt(5); i < l; i++) {
            byte[] name = new byte[10];
            r.nextBytes(name);
            Fluid f = new GT_Fluid("gt.spamfluid." + (new String(name)),"molten.autogenerated", Dyes._NULL.mRGBa);
